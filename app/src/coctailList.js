...
renderItem = ({ item }) => (
  ...
  />
   <TouchableOpacity style={styles.button} onPress={() =>  Linking.openURL(item.artist.url)}>
      <Text
        style={{fontSize:13, marginLeft: 16, color:"#fa926f", textDecorationLine:"underline"}}>
        На страницу артиста ...
       </Text>
    </TouchableOpacity>
  </View>
   ...
   return this.props.list.loading == true ? (
        ...
        <SafeAreaView>
          <ScrollView style={{backgroundColor:"#1b1b49"}}>
            ...
            {this.props.list.users && this.props.list.users.data && this.props.list.users.data.tracks.track && (
              <FlatList
                style={{padding: 16}}
